...
func (s *server) createVenue(w http.ResponseWriter, r *http.Request) {
	...
	if environment == 0 {
		if strings.TrimSpace(body.APIKey) != "" || body.CredentialInfo != nil {
			writeErr(w, http.StatusBadRequest, "backtest venue credentials are generated by core-service")
			return
		}
	} else if body.Spot != nil || body.Futures != nil {
		writeErr(w, http.StatusBadRequest, "venue wallet bootstrap is only supported for backtest venues")
		return
	}
	...
	if environment == 0 {
		if market == 1 {
			...
			bootstrap, err = buildWalletBootstrap(nil, body.Futures)
		}
		if err != nil {
			writeErr(w, http.StatusBadRequest, err.Error())
			return
...
func venueCredentialJSON(raw json.RawMessage) (string, error) {
